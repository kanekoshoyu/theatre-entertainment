app.controller('UserProfileCtrl', function($q, $routeParams, $rootScope, $location, $scope, $timeout, $translate, Geolocator, UserService, Restangular, CreateCampaignService, FileUploadService, PortalSettingsService, PHONE_TYPE) {
  ...
  PortalSettingsService.getSettingsObj().then(function(success) {
    ...
    // get custom fields
    $scope.custom_field = [];
    Restangular.one('account/setting').customGET().then(function(success) {
      if (success) {
      ...
      if ($scope.public_settings.site_campaign_business_section_custom) {
        if ($scope.public_settings.site_campaign_business_section_custom.length > 0) {
          ...
          angular.forEach($scope.public_settings.site_campaign_business_section_custom, function(value) {
            ...
            angular.forEach($scope.custom_field, function(val) {
              if (val.name) {
      ...
      if ($scope.public_settings.site_campaign_personal_section_custom) {
        if ($scope.public_settings.site_campaign_personal_section_custom.length > 0) {
          $scope.pcustom = [];
          angular.forEach($scope.public_settings.site_campaign_personal_section_custom, function(value) {
            var fieldRequire = false;
            var fieldPlaceholder = '';
            if(value.placeholder) {
              fieldPlaceholder = value.placeholder;
  ...
  function loadCampaign() {
    ...
    CreateCampaignService.load($routeParams.campaign_id).then(function(success) {
      ...
      if ($scope.campaign.business_organizations) {
        ...
        $scope.getDescription($scope.company_selected);
  ...
  $scope.getDescription = function(bid) {
  ...
  var personalValidation = function() {
    ...
    $('form').form($scope.form_validation, {
      inline: true,
  ...
  $scope.saveData = function($event) {
    ...
    if ($scope.valcheck) {
      ...
      $scope.custom_field = $scope.custom_field == undefined ? [] : $scope.custom_field;
      if ($scope.custom_field.length > 0) {
        var delObj = {};
        if ($scope.custom_field) {
          $scope.custom_field.forEach(function(value) {
            delObj[value.name] = 1;
          });
          Restangular.one('account').customDELETE("setting", delObj).then(function(success) {
            updateAccSetting(custom_setting);
            //('success');
          }, function(failed) {
            updateAccSetting(custom_setting);
          });
      ...
      function updateAccSetting(settingObj) {
        for (var i in settingObj) {
          if (settingObj.hasOwnProperty(i) && !settingObj[i]) {
            delete settingObj[i];
          }
        }
        if (Object.getOwnPropertyNames(settingObj).length) {
          Restangular.one('account/setting').customPUT(settingObj);
        }
      }
        ...
        // add new company
          ...
          if ($scope.companyFormToggle == false) {
            Restangular.one('account/business').customPOST(data).then(function(success) {
              ...
              fileParam.business_organization_id = success.id;
  ...
  $scope.modifyBusinessImage = function(files, params) {
    FileUploadService.upload('account/resource/file', files, params).then(function(success) {
      $scope.businessImage = [];
      $scope.businessImage.push(success[0].data);
    });
  }
  ...
  $scope.newOrExistComp = function() {
    ...
    if (!$scope.companyFormToggle) {
      ...
      $scope.selectedSubcountry.selected = false;
      $scope.selectedCity.selected = false;
