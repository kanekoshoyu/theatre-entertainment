app.controller('ExploreCtrl', function($timeout, $scope, $rootScope, CampaignSettingsService, $routeParams, $translatePartialLoader, $translate, $location, $route, RESOURCE_REGIONS, Restangular, Geolocator, RestFullResponse, PortalSettingsService, RequestCacheService, VideoLinkService) {
  var nativeLookup;
  ...
  PortalSettingsService.getSettingsObj().then(function(success) {
    ...
    if ($scope.setDefaultSort.default == 'default' && $scope.sortOrFilters.sort == '' && typeof $routeParams.sort == 'undefined') {
      $timeout(function() {
