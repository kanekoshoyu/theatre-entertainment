app.controller('HomeCtrl', function($timeout, $scope, $route, $rootScope, $location, CampaignSettingsService, $translatePartialLoader, $translate, CreateCampaignService, Restangular, RESOURCE_REGIONS, PortalSettingsService, RequestCacheService, TimeStatusService, $sce, VideoLinkService, UserService, $q) {
  var nativeLookup;
  //Thrinacia home page
  ...
  PortalSettingsService.getSettingsObj().then(function(success) {
    ...
    get_recent_campaigns(success.public_setting.recentProjectHideFeatured, success.public_setting.exclude_ended_from_recent, success.public_setting.campaign_randomize);
  ...
  Restangular.all('campaign').getList().then(function(success) {
    $scope.allcampaigns = success;
  ...
  var get_recent_campaigns = function(recentProjectHideFeatured, exclude_ended_from_recent, campaign_randomize) {
    ...
    if (campaign_randomize) {
      recent_campaigns_filters["sort"] = recent_campaigns_filters["sort"].replace("-created", "*random()");
    }

    // get recent campaigns
    Restangular.all('campaign').getList(recent_campaigns_filters).then(function(success) {
      $scope.recentcampaigns = success;
  ...
  var getFeaturedCampaigns = function() {
    Restangular.all('campaign').getList({
      ...
      $scope.featuredcampaigns = success;
